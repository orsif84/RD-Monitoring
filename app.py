...
import httpx
import time
from portkey_ai.exceptions import APITimeoutError
...
Context Information:
{context}
Please provide a risk score (0-1) and rationale.
Format:
Score: <number>
Rationale: <text>
"""
    ...
    while attempt < retries:
        try:
            ...
            st.warning(f"API Timeout, retrying {attempt+1}/{retries}...")
            time.sleep(wait)
            attempt += 1
...
def fallback_risk(event_type, event):
    # heuristic fallback, same as previous
    risk = 0.0
    reasons = []
    if event_type == "Clinical Trial":
        ...
        if 'PHASE 1' in phase:
            risk += 0.5; reasons.append("Early phase 1")
        elif 'PHASE 2' in phase:
            risk += 0.3; reasons.append("Phase 2")
        if isinstance(enrollment, (int,float)):
            if enrollment < 30:
                risk += 0.25; reasons.append(f"Low enrollment ({enrollment})")
            elif enrollment < 100:
                risk += 0.1; reasons.append(f"Medium enrollment ({enrollment})")
            else:
                risk -= 0.05; reasons.append("Healthy enrollment")
        ...
        reasons.append("Text info present")
        if any(k in summary for k in ["warning", "safety", "risk", "terminated", "adverse", "recall"]):
            risk = max(risk, 0.5)
            reasons.append("Risk-related keywords found")
        rationale = "Fallback heuristic due to missing LLM score; " + "; ".join(reasons)
        return min(max(risk, 0),1), rationale
    return 0.1, "Fallback heuristic applied"

def build_timeline_df(portkey, trials_df, press_df):
...
def main():
    ...
    timeline_df = build_timeline_df(portkey, trials_df, press_df)
    ...
    def risk_color(val):
        if pd.isnull(val):
            return ''
        if val > 0.7:
            return 'background-color: #fa8072; color: #fff; font-weight: bold;'
        elif val > 0.4:
    ...
    for et in timeline_df['EventType'].unique():
        d = timeline_df[timeline_df['EventType'] == et]
        ax.plot(d['Date'], d['RiskScore'], color=colors[et], marker=markers[et], label=et, linestyle='-')
        for _, row in d.iterrows():
            label = row['Title'] if len(row['Title']) <= 30 else row['Title'][:27] + '...'
            ax.annotate(label, (row['Date'], row['RiskScore']), textcoords='offset points', xytext=(0,10), ha='center', fontsize=8, color=colors[et])
    ax.set_ylim(0, 1.1)
